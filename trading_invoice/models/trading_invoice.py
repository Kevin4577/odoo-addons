...
class TradingInvoice(models.Model):
    ...
    def get_order_lines(self, stock_picking):
        ...
        for index, line in enumerate(order_lines):
            product_lines.append({'index': index,
    ...
    def get_product_lot_list_per_sale_order(self, stock_picking_list):
        ...
        for sale_order in sale_order_list:
            ...
            for operation_line in operation_lines_per_sale_order:
                for operation_lot_line in operation_line.pack_lot_ids:
                    product_lines.append({'shipping_marks': '',
        ...
        return {'sum_product_qty': sum_product_qty,
    ...
    def get_product_order_list_with_qty(self, stock_picking_list):
        ...
        return {'sum_qty': sum_qty,
                ...
                'payment_term_id':
                stock_picking_list[0].sale_id.payment_term_id,
        ...
        for sale_order in sale_order_list:
            for sale_order in sale_order_list:
                ...
                for operation_line in operation_lines_per_sale_order:
                    ...
                    for operation_lot_line in operation_line.pack_lot_ids:
                        product_lines.\
                            append({'package_no': package_number,
                                    'client_order_ref': client_order_ref,
                                    'customer_product_code':
                                    operation_line.
                                    product_id.customer_product_code,
                                    'qty': operation_lot_line.qty,
                                    'carton_qty':
                                    operation_lot_line.lot_id.carton_qty,
                                    'gw_lot':
                                    operation_lot_line.lot_id.gross_weight,
                                    'means_lot':
                                    operation_lot_line.lot_id.volume,
                                    })
        sum_qty = 0.0
        pallet_sum = 0.0
        sum_gw = 0.0
    ...
    def get_order_lines_per_invoice(self, account_invoice):
        ...
        for sale_order in sale_order_list:
            account_invoice_lines_per_same_order = \
                ...
                filtered(lambda line: sale_order.id in line.sale_line_ids.
                         mapped('order_id').ids)
            sale_order_lists = []
            sale_order_lists.append({'sale_order_name': sale_order.name,
                                     'client_order_ref':
        ...
        return {'sale_order_list': sale_order_lists,
                'sum_qty': sum_qty,
                'sum_amount': sum_amount,
        ...
        for package in package_list:
            ...
            for sale_order in sale_order_list:
                ...
                for pack_operation_lot in\
                    ...
                    order_lines.\
                        append({'carton_no':
                                pack_operation_lot.lot_id.carton_no,
                                'customer_product_code':
                ...
                order_list.append({'name': sale_order.name,
                                   'client_order_ref':
                                   sale_order.client_order_ref,
                                   'lines': order_lines
                                   })
            ...
            package_list = []
            package_list.append({'order_list': order_list,
                                 'pallet_sum': pallet_sum,
    ...
    def get_package_name_per_package_list(self, stock_picking_list):
        ...
        for package_type in package_type_list:
            ...
            package_list_per_same_package_type = \
                ...
                mapped('result_package_id')
            package_list.\
                append({'name': package_type.name,
                        'lines_number':
                        len(pack_operation_lot_list_per_same_package_type),
                        'list':
                        package_list_per_same_package_type.mapped('name'),
                        })
        return package_list
    ...
    def get_pack_lot_list_per_package_type(self, stock_picking_list):
        ...
        for package_type in package_type_list:
            ...
            for pack_lot in pack_operation_lot_list_per_same_package_type:
                pack_lot_list = []
                pack_lot_list.\
                    append({'client_order_ref':
                            pack_lot.operation_id.picking_id.sale_id.
                            client_order_ref,
                            'product_id': pack_lot.operation_id.product_id,
                            'product_uom':
                            pack_lot.operation_id.product_uom_id.name,
