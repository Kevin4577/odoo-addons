...
class TestInvoiceReference(common.TransactionCase):

    def setUp(self):
        super(TestInvoiceReference, self).setUp()
        ...
        self.sale_order.picking_ids[0]. \
            pack_operation_ids[0].result_package_id = self.pack1
        self.sale_order.picking_ids[0].pack_operation_product_ids.write({
            'qty_done': 5.0
        })
        self.packaging_id = self.product_packaging_model.create({
            'name': 'Test box of 10'
        })
        self.sale_order.picking_ids[0].pack_operation_product_ids[0]. \
    ...
    def test_account_invoice_name_get(self):
        """Test name get method for invisible hs name"""
        self.sale_order.invoice_ids.name_get()
