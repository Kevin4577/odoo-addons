...
class TestStockValuation(common.TransactionCase):
    ...
    def test_10_render_report_with_product(self):
        self.stock_valuation.fields_view_get()
        self.stock_valuation._selection_filter_by_product()
        self.stock_valuation._selection_filter_by_order_type()
        self.stock_valuation.onchange_filter_by_product()
        self.stock_valuation.onchange_filter_by_order_type()
    ...
    def test_20_render_report_with_product_category(self):
        self.stock_valuation.write({
            ...
            'partner_customer_id': self.partner_id.id
        })
        context = self.stock_valuation.prepare_valuation()
        context['context'].update(uid=self.uid)
        self.stock_valuation_list = \
    ...
    def test_30_render_report_with_product_lot(self):
        self.stock_valuation.write({
            'lot_id': self.lot1.id,
            'involved_mrp_order': self.mrp_order1.id
        })
        context = self.stock_valuation.prepare_valuation()
        context['context'].update(uid=self.uid)
        self.stock_valuation_list = \
    ...
    def test_40_render_report_with_product_package(self):
        self.stock_valuation.write({
