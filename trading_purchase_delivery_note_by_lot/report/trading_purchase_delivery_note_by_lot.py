...
@py3o_report.py3o_report_extender(
    report_xml_id='trading_purchase_delivery_note_by_lot.'
    'trading_purchase_delivery_note_by_lot_py3o')
def render_report_with_data(report_xml_id, ctx):
    ...
    if not stock_picking_list.filtered(lambda stock_picking:
                                       not stock_picking.pruchase_id):
        data = base_invoice_export_obj.get_product_lot_list_per_sale_order(
            stock_picking_list)
        ctx.update(data)
    else:
